...
"""
    ...
    Example:
    model = get_efficientnet('efficientnet-b7')
"""
    ...
    def extract_endpoints(self, inputs):
        ...
        for idx, block in enumerate(self._blocks):
            ...
            if prev_x.size(2) > x.size(2):
                ...
                lateral_features.append(prev_x)
            prev_x = x

        # Head
        x = self._swish(self._bn1(self._conv_head(x)))
        # endpoints['reduction_{}'.format(len(endpoints) + 1)] = x
        lateral_features.append(x)
        lateral_features.pop(0)
...
class get_efficientnet(nn.Module):
    def __init__(self, name, pretrained=True):
        super(get_efficientnet, self).__init__()
        if pretrained:
...
if __name__ == "__main__":
    ...
    model = get_efficientnet("efficientnet-b7", False)
    img = torch.rand((5, 3, 256, 256))
    print(model)
    # for key, value in model.extract_endpoints(img).items():
    #     print(value.shape)
    # for i in model(img):
    #     print(i.shape)
